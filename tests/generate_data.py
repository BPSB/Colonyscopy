...
def colony_centres(format,resolution):
	"""
	like midpoints, in two dimensions: splits an an area of size `resolution` into a checkerboard with `format[0]`×`format[1]` elements.
	Returns a 3D array, containing the coordinates of the centres of each tile.
	"""
	return np.dstack(np.meshgrid(*[
			midpoints(format[i],resolution[i]) for i in (0,1)
		],indexing="ij"))
...
def generate_data(colony_sizes,resolution,bg=(0,0,0),fg=(255,255,255)):
	...
	format = colony_sizes.shape[1:3]
	positions = colony_centres(format,resolution)
...
def test_generate_data():
	times = 4
	format = (5,7)
	colony_sizes = np.random.uniform(3,10,(times,*format))
	resolution = (200,100)
	...
	centres = colony_centres(format,resolution)
	for _ in range(1000):
		...
		is_colony = any(
				...
				for i,j in product(*map(range,format))
			)

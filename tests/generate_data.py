...
def colony_centres(dimensions,resolution):
	"""
	like midpoints, in two dimensions: splits an an area of size `resolution` into a checkerboard with `dimensions`.
...
def generate_data(colony_sizes,resolution,bg=(0,0,0),fg=(255,255,255)):
	...
	dimensions = colony_sizes.shape[1:3]
	positions = colony_centres(dimensions,resolution)
...
def test_generate_data():
	times = 4
	dimensions = (5,7)
	colony_sizes = np.random.uniform(3,10,(times,*dimensions))
	resolution = (200,100)
